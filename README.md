...
### ＨＰＥ ｉＬＯカスタム
* ＣＳＲ生成
  * `certmgr [-c config_file] hpeilo_generate [-m][-u][-t TYPE][-s SIGN][--sans=<ドメイン>[,<ドメイン>...][--ocsp-must-staple] <ｉＬＯサーバー名>`
* ＳＳＬ証明書のデプロイ
  * `certmgr [-c config_file] hpeilo_deproy <証明書ＩＤ|コモンネーム>`
...
* 重大バグ
  * ｉＬＯにアクセスするためのＩＤ・パスワード設定が（外部インターフェースが無くて）できません。
